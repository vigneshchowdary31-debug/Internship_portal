...
class GoogleService {
    static getOAuthClient() {
        ...
        if (!clientId || !clientSecret || !redirectUri) {
            ...
            return new googleapis_1.google.auth.OAuth2('mock-client-id', 'mock-client-secret', redirectUri || 'https://mock-redirect-uri.local/callback');
        }
    ...
    static async createMeetEvent(title, description, startTime, endTime) {
        ...
        if (!process.env.GOOGLE_REFRESH_TOKEN) {
            ...
            const mockCode = (0, uuid_1.v4)().substring(0, 10);
            return {
                eventId: `mock-event-${(0, uuid_1.v4)()}`,
    ...
    static async updateMeetEvent(eventId, title, description, startTime, endTime) {
        if (!process.env.GOOGLE_REFRESH_TOKEN)
            return true; // Mocked
        const auth = this.getOAuthClient();
        const calendarId = process.env.GOOGLE_CALENDAR_ID || 'primary';
    ...
    static async deleteMeetEvent(eventId) {
        if (!process.env.GOOGLE_REFRESH_TOKEN)
            return true; // Mocked
        const auth = this.getOAuthClient();
        const calendarId = process.env.GOOGLE_CALENDAR_ID || 'primary';
